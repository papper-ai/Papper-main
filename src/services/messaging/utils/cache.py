...
class MessagingCache:
    ...
    async def get_chat(self, chat_id: uuid.uuid4) -> None | ChatPayload:
    ...
    async def set_chat(self, chat_id: uuid.uuid4, chat_payload: ChatPayload) -> bool:
        str_chat_payload = chat_payload.model_dump_json()
        return await self.redis.set(
            f"{self.cache_prefix}{self.chat_prefix}{chat_id.hex}", str_chat_payload
        )

    async def delete_chat(self, chat_id: uuid.uuid4) -> bool:
    ...
    async def get_chats(
        self, id: uuid.uuid4, is_archived: bool
        ...
        chat_payloads = (
            [ChatPayload(**chat) for chat in result] if json_result else None
        )
        return chat_payloads

    async def set_chats(
        self, id: uuid.uuid4, chat_payloads: list[ChatPayload], is_archived: bool
    ) -> bool:
        str_chat_payloads = [
    ...
    async def delete_chats(self, id: uuid.uuid4) -> None:
